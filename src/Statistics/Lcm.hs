...
-- @
lcm :: (Real a, Real b, Show a, Vector v a, Show b, Vector v b, Vector v Bool)
    => v a -> v b -> ([a], [b], [Double])
lcm ps rs | lPs /= lRs = error $ "lcm: Number of predictors is " ++ show lPs
                         ++ ", but number of responses is " ++ show lRs ++ "."
          | not (strictlyOrdered ps) = error "lcm: The predictors are not strictly ordered."
          | otherwise = unsafeLcm ps rs
...
unsafeLcm :: (Real a, Real b, Show a, Vector v a, Show b, Vector v b)
    => v a -> v b -> ([a], [b], [Double])
unsafeLcm ps rs | l == 0    = ([], [], [])
                | l == 1    = start
